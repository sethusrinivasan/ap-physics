...
Use this file to restart the same workflow for a new problem. Copy the prompt below into a new Cursor chat, or mention `@PROMPT.md` and attach the problem PDF(s).
...
- Previous / Next problem links, plus a Contents link back to that unit’s table of contents (`index_unitN.html`).
- After the new problem exists, add it to `index_unitN.html` and wire Previous/Next between neighbors. If this is the first problem in a new unit, also add the unit to the root `index.html`.
...
6. **Required:** add a row for the new problem to `index_unitN.html` in the same pass. Do not leave the unit TOC listing only older problems. New units also get a row on the root `index.html`.
7. Update Previous/Next on the neighboring walkthroughs. Each walkthrough nav should look like: Previous | Contents (`../index_unitN.html`) | Next.
